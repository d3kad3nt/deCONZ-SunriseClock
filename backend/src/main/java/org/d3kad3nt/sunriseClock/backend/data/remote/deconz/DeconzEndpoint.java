...
import org.d3kad3nt.sunriseClock.backend.data.remote.deconz.typeadapter.RemoteGroupListTypeAdapter;
import org.d3kad3nt.sunriseClock.backend.data.remote.deconz.typeadapter.RemoteGroupTypeAdapter;
import org.d3kad3nt.sunriseClock.backend.data.remote.deconz.typeadapter.RemoteLightListTypeAdapter;
import org.d3kad3nt.sunriseClock.backend.data.remote.deconz.typeadapter.RemoteLightTypeAdapter;
...
public class DeconzEndpoint extends BaseEndpoint {
    ...
    /**
     * From the Deconz rest api documentation: "An 'all' group is generated by the REST API by default with all lights
     * in it. This group is available at '/api/groups/0' but this id is not always 0. In that case the group is also
     * available at 'api/groups/65520'.
     */
    ...
    public DeconzEndpoint init() {
        ...
        Gson gson = new GsonBuilder()
                .registerTypeAdapter(remoteLightType, new RemoteLightTypeAdapter(endpointId))
                ...
                .registerTypeAdapter(remoteGroupListType, new RemoteGroupListTypeAdapter(endpointId))
                .create();
    ...
    @Override
    public LiveData<ApiResponse<RemoteGroup>> getGroup(String groupId) {
        LogUtil.d("Requesting single group with id %s.", groupId);
        return this.retrofit.getGroup(groupId);
    }

    @Override
    public LiveData<ApiResponse<ResponseBody>> setGroupOnState(String groupId, boolean on) {
        LogUtil.d("Setting group state for id %s to %s.", groupId, on);
        JsonObject requestBody = new JsonObject();
        requestBody.add("on", new JsonPrimitive(on));
        return this.retrofit.updateGroupState(groupId, requestBody);
    }
}
