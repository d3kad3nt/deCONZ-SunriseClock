...
public interface DbLightDao extends DbEndpointEntityDao<DbLight> {
    ...
    @Query(
            value = "SELECT id FROM '" + DbLight.TABLENAME + "' WHERE endpoint_id = :endpointId AND id_on_endpoint ="
                    + " :endpointLightId")
    ...
    @Query(
            value = "SELECT * FROM light " + "INNER JOIN light_grouping ON light_grouping.light_id = light.id "
