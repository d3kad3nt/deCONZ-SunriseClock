...
public interface DbGroupDao extends DbEndpointEntityDao<DbGroup> {
    ...
    @Query(
            value = "SELECT id FROM '" + DbGroup.TABLENAME + "' WHERE endpoint_id = :endpointId AND id_on_endpoint ="
                    + " :endpointGroupId")
    ...
    @Query(
            value = "SELECT * FROM `group` " + "INNER JOIN light_grouping ON light_grouping.group_id = `group`.id "
