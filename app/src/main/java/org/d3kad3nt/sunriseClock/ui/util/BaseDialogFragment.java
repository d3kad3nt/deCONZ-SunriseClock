...
public abstract class BaseDialogFragment<
  ...
  public View onCreateView(
    ...
    // cases,
    // this means that we don't need to override onCreateDialog(), as the default empty dialog is
    // populated with
    // our view.
    return binding.getRoot();
  }

  // onViewCreated() is only called because we've overridden onCreateView() and provided a non-null
  // view.
  @Override
   ...
   *
   * <p>The binding class is available under {@link #binding}. Typically, the developer will be able
   * to call the subclass's set method directly. For example, if there is a variable x in the
   ...
   * viewLifecycleOwner as the LifecycleOwner in a DialogFragment that uses Dialog objects, instead,
   * use the the DialogFragment itself, or, if you're using Jetpack Navigation, use the
   * NavBackstackEntry.
   */
  protected abstract LifecycleOwner observeData();

  /**
   * Creates ViewModelProvider, used to create VM instances and retain them in the ViewModelStore of
   * the given ViewModelStoreOwner.
   */
