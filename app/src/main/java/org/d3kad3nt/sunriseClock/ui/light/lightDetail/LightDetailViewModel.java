...
public class LightDetailViewModel extends ViewModel {
  ...
  /**
   * Text that is shown in the light rename dialog. The user types the desired new name into a text
   * field backed by this LiveData.
   */
  ...
  public LightDetailViewModel(@NonNull LightRepository lightRepository, long lightId) {
    ...
    loadingIndicatorVisibility =
        new ResourceVisibilityLiveData(View.VISIBLE)
    ...
    notReachableCardVisibility =
        new BooleanVisibilityLiveData(View.GONE)
            .setTrueVisibility(View.GONE)
            .setFalseVisibility(View.VISIBLE)
            .addVisibilityProvider(getIsReachable());

    // If the light name changes upstream, we update the name that the user is getting shown in the
    // rename dialog.
    lightNameEditText =
