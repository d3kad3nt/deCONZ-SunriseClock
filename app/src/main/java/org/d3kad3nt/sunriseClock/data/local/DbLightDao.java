...
public interface DbLightDao {
  ...
  /**
   * Insert light object into the database (create) or update existing light object. Case 1: Insert
   * if neither lightId (primary key) nor (endpointId and endpointLightId) are found inside the
   * database. Case 2: Update if lightId (primary key) is given in the light object. A light with
   * this lightId must already exist inside the database. Case 3: Update if endpointId and
   * endpointLightId are given in the light object. A light with this endpointId and endpointLightId
   * must already exist inside the database.
   *
  ...
  default void upsert(DbLight obj) {
    ...
    // Case 3
    // Primary key NOT found in light object. This can happen if (all) light objects are retrieved
    // from the
    // remote endpoint.
  ...
  /**
   * Insert the light into the database. If there is a conflict on insert, the light is not inserted
   * and a special value is returned.
   *
   ...
   * instances to rows in the database. If there is no row with the same primary key, Room makes no
   * changes.
   *
