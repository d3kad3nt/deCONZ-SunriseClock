...
public interface DbLightDao {
    ...
    default void upsert(DbLight obj) {
        ...
        if (rowId != -1L) {
            Log.d(TAG,
                    "Inserted DbLight (row id: " + rowId + ") with endpointId " + obj.getEndpointId() + " and " +
        ...
        if (obj.getLightId() != 0L) {
            ...
            if (rowsUpdated >= 1) {
                ...
                Log.w(TAG,
                        "0 rows updated by room. This could mean that the primary key (lightId: " + obj.getLightId() + ") could not be found in the database table.");
            }
        }
        // Case 3
        ...
        // Rhe lightId primary key is autogenerated by room, therefore it is not known to the remote endpoint.
            ...
            int rowsUpdated = updateUsingEndpointIdAndEndpointLightId(obj.getEndpointId(), obj.getEndpointLightId()
                    , obj.getName(), obj.getIsSwitchable(), obj.getIsOn(), obj.getIsDimmable(), obj.getBrightness()
                    , obj.getIsTemperaturable(), obj.getColorTemperature(), obj.getIsColorable(), obj.getColor());
            Log.d(TAG,
                    rowsUpdated + " rows updated by room. Updated DbLight with endpointId: " + obj.getEndpointId() + " and endpointLightId: " + obj.getEndpointLightId());
        }
        else {
            Log.w(TAG, "Neither lightId nor (endpointId and endpointLightId) were set. No update could be " +
                    "performed" + " by room!");
        }
    }
