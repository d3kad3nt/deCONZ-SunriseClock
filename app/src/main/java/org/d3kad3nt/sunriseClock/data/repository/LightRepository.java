...
public class LightRepository {
  ...
  /**
   * Using singleton pattern as of now. With dependency injection (Dagger, ...) this class could be
   * mocked when unit testing. TODO: Dependency Injection, optional
   */
  ...
  public LiveData<EmptyResource> refreshLight(long lightId) {
    ...
    return Transformations.map(
        new NetworkBoundResource<Empty, RemoteLight, DbLight>() {
          ...
          protected void saveResponseToDb(DbLight item) {
            // The primary key lightId is not known to the remote endpoint, but it is known to us.
            // Set the lightId to enable direct update/insert via primary key (instead of endpointId
            // and
            // endpointLightId) through Room.
            item.setLightId(lightId);
            dbLightDao.upsert(item);
   ...
   *
   * @param endpointId The endpoint on which to execute the action. The given ID must already exist
   *     in the database.
   * @return Resource representing the status of the request.
   */
  ...
  /**
   * Changes the brightness of the light. TODO: Define whether a brightness of 0 means off or lowest
