...
public class RemoteLightTypeAdapter implements JsonDeserializer<RemoteLight> {
   ...
   *     part of the JSON response, therefore it has to be set manually for a specific DbLight when
   *     deserializing it.
   */
  ...
  public RemoteLight deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    // When requesting a list of RemoteLights, the endpointLightId is manually added to the json,
    // too.
    // This enables the existing Gson typeadapter (RemoteLightTypeAdapter) to work for requests for
    // both single
