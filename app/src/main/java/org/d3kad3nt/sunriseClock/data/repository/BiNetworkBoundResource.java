...
/**
 * A generic class that can provide a resource backed by both the sqlite database and TWO network
 * operations.
 */
abstract class BiNetworkBoundResource<ResultType, RemoteType1, RemoteType2, DbType>
    ...
    /**
     * A generic class that can provide a resource backed by both the sqlite database and TWO
     * network operations.
     */
    ...
    protected DbType convertRemoteTypeToDbType(final ApiSuccessResponse<RemoteType1> response) {
        throw new UnsupportedOperationException(
                "The method convertRemoteTypeToDbType() should not be called on "
        ...
        if (ApiSuccessResponse.class.equals(aClass) && ApiSuccessResponse.class.equals(bClass)) {
            ServiceLocator.getExecutor(ExecutorType.IO).execute(() -> {
                saveResponseToDb(convertRemoteTypeToDbType(
                        (ApiSuccessResponse<RemoteType1>) data1,
            ...
            addSource(dbSource, newData -> {
                updateValue(Resource.error(
                        ((ApiErrorResponse<RemoteType1>) data1).getErrorMessage(),
            ...
            addSource(dbSource, newData -> {
                updateValue(Resource.error(
                        ((ApiErrorResponse<RemoteType2>) data2).getErrorMessage(),
                        convertDbTypeToResultType(newData)));
            });
    ...
    /**
     * Load additional fresh data from the network, for example by calling methods on the endpoint
     * (backed e.g. by Retrofit network requests):
     *
     * <p>{@code return endpoint.getGroup(dbObject.getEndpointEntityId());}
     *
     * <p>This is the second remote request and triggered right after {@link #loadFromNetwork()}.
     * The result of both methods can be used to construct a unified database entity in
    ...
    /**
     * Convert from two remote (network) data transfer objects to a object suitable for database
     * inserts or updates. This 'combined data' is given directly to
     * {@link #saveResponseToDb(DbType)}.
     *
     * @param response The response for the first network request, triggered in
     *     {@link #loadFromNetwork()}.
     * @param response2 The response for the second network request, triggered in
     *     {@link #loadFromNetwork2()}.
     */
    protected abstract DbType convertRemoteTypeToDbType(
            ApiSuccessResponse<RemoteType1> response,
