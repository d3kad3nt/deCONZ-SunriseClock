...
public interface DbEndpointEntityDao <T extends DbEndpointEntity> {
     ...
     *                         {@link DbEndpointEntity#getEndpointEntityId()}), the composite
     *                         primary key) set.
    ...
    default long upsert(T dbEndpointEntity) {
        // Case 1
        long rowId = save(dbEndpointEntity); if (rowId != -1L) {
            LogUtil.d("Inserted into table %s with endpointId %d and endpointEntityId %s. This resulted in the " +
                    "primary key (id) to be set to %d.",
                dbEndpointEntity.getTABLENAME(), dbEndpointEntity.getEndpointId(),
                dbEndpointEntity.getEndpointEntityId(), rowId);
            // Per the SQLite specification the primary key (id) becomes an alias for the row id if the table
        ...
        long dbEndpointEntityId = dbEndpointEntity.getId(); if (dbEndpointEntityId != 0L) {
            int rowsUpdated = updateUsingPrimaryKey(dbEndpointEntity); if (rowsUpdated >= 1) {
                LogUtil.d("Updated %d rows. Updated table %s with primary key %s.", rowsUpdated,
                    dbEndpointEntity.getTABLENAME(), dbEndpointEntityId);
                return dbEndpointEntityId;
            } else if (rowsUpdated == 0) {
                LogUtil.w("Updated 0 rows. This could mean that the primary key %s could not be found in the " +
                    "%s table.", dbEndpointEntityId, dbEndpointEntity.getTABLENAME());
                return 0;
            }
        }
        ...
        if (endpointId != 0L && !(endpointEntityId.isEmpty())) {
            ...
            LogUtil.d("Updated %d rows. Updated table %s with endpointId %d and endpointEntityId %s.", rowsUpdated,
                dbEndpointEntity.getTABLENAME(), endpointId, endpointEntityId);
            // Unfortunately Room does not support the RETURNING clause in SQLITE which could be used to return the
            // primary key of the newly updated database entity. We have to resort to send another SELECT query to
            // the database.
            // Todo: To improve performance we could implement two separate upsert() methods to only return the
            //  primary key (id) when needed.
            long pk = getIdForEndpointIdAndEndpointEntityId(endpointId, endpointEntityId);
            LogUtil.v("This UPDATE operation led to a separate SELECT query on the %s table to get the primary key " +
                "%d of the database entity.", dbEndpointEntity.getTABLENAME(), pk);
            return pk;
        } else {
     ...
     * @return The primary key for the dbEndpointEntity identified by the given composite key. 0 if the primary
     * could not be found.
     */
