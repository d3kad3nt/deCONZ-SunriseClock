    ...
    default void upsert(DbLight obj) {
        ...
        if (rowId != -1L) {
            Log.d(TAG, "Inserted DbLight (row id: " + rowId + ") with endpointId " + obj.getEndpointId() + " and " +
                "endpointLightId: " + obj.getEndpointEntityId());
            return;
        }
        ...
        if (obj.getId() != 0L) {
            ...
            if (rowsUpdated >= 1) {
                Log.d(TAG, rowsUpdated + " rows updated by room. Updated DbLight with lightId: " + obj.getId());
            } else if (rowsUpdated == 0) {
                Log.w(TAG, "0 rows updated by room. This could mean that the primary key (lightId: " + obj.getId() +
        ...
        // Rhe lightId primary key is autogenerated by room, therefore it is not known to the remote endpoint.
        else if (obj.getEndpointId() != 0L && !(obj.getEndpointEntityId().equals(""))) {
            int rowsUpdated =
                updateUsingEndpointIdAndEndpointLightId(obj.getEndpointId(), obj.getEndpointEntityId(), obj.getName(),
            ...
            Log.d(TAG,
                rowsUpdated + " rows updated by room. Updated DbLight with endpointId: " + obj.getEndpointId() +
                    " and endpointLightId: " + obj.getEndpointEntityId());
        } else {
            Log.w(TAG,
