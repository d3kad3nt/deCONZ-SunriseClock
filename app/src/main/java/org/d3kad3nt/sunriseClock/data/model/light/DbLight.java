...
@Entity(
    ...
    indices = {
      @Index(
          value = {"endpoint_id", "endpoint_light_id"},
          unique = true)
    },
...
public class DbLight {
  ...
  @NonNull
  // Set SQLITE notNull attribute, for primitive types this is set automatically (but this is a
  // string).
  ...
  /**
   * Create a new object that represents a light in the app's Room database. This constructor has to
   * be public for Room to be able to create an object. This should not be otherwise accessed!
   */
  public DbLight(
      long endpointId,
      @NonNull String endpointLightId,
      @NonNull String name,
    ...
    this.colorTemperature =
        colorTemperature; // Todo: Define which values are allowed, see DbLightBuilder javadoc
  ...
  /**
   * This field enables the remote endpoint to identify the correct light. A remote endpoint cannot
   * work with the lightId.
   *
  ...
  /**
   * @return Whether the light's capabilities allow it to be dimmed (true if allowed by the device,
   *     false if not).
   */
  ...
  /**
   * @return The current brightness of the light, where 0 is the lowest brightness or off (depending
   *     on the light) and 100 is the highest brightness.
   */
  ...
  /**
   * @return Whether the light's capabilities allow its color to be changed (true if allowed by the
   *     device, false if not).
   */
